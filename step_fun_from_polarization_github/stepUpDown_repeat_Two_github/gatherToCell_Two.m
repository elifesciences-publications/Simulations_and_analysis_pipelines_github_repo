function [rxnx, rxny, rxnx_tofit, rxny_tofit, rxnstep, rxnlabel, ...
    rxnmu, rxnsigma] = ...
    gatherToCell_Two(rxnset, steptime, delayTime, AllRxns, ActTime, PolData, TONORM)
...
for r=1:numel(rxnset)
    ...
    %make sure 16 hrs have passed since step before fitting
    if ~isnan(steptime(r))
        goodt = (t - steptime(r)) > 16;
    else %control rxns have stepTime = nan, so just wait for 2 hrs after step
        goodt = t >= delayTime;
    end
    ...
    if strcmp(rxnset{r},'sD8') || strcmp(rxnset{r},'sD16')
        rxnstep{r} = nan;
        rxnlabel{r} = 'control';
    elseif strcmp(rxnset{r},'sU8') || strcmp(rxnset{r},'sU16')
        rxnstep{r} = nan;
        rxnlabel{r} = 'control';
    else
