...
dO = makeOrbit(1, 0, aval(pickA),epsval(pickEps), OMSIGN*2*pi/24);
nO = makeOrbit(Rval(pickR), Xval(pickX), aval(pickA), epsval(pickEps), OMSIGN*2*pi/24);

[t, polTr, xyTr, dn] = ...
    entrain(dO, nO, tauval(pickTau), 24, NUMCYC, [dO.R theta0(pickTheta)]);
[tLL, polLL, release] = ...
    integrateTraj(dO, polarCoordOn(dO,xyTr(end,:)), ...
    0:0.1:squeeze(PEAKT(pickEps,pickR,pickX,1,pickTau)));
