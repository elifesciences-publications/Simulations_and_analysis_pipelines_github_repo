...
%oscillator first hits the orbit; time2circle = time
%from start point to time of hitting the orbit.
%appPh0 = apparent phase at end of entrainment, computed by
% appPh0=reachPhase-time2reachPhase*omega; pretty meaningless when omega is
% nonconstant!!! Don't use!

%For varyingSpeed simulations with dtheta/dt = omega + eps*sin(omega*t),
%this function integrates motion along the orbit explicitly to find the
%first point when the oscillator hits pi/2.
...
while ~done
    ...
    if ~isempty(time2orbit_temp)
        ...
        %integrate for 24 hours (2*pi/orbit.omega) to determine at what
        %time the oscillator reaches pkPhase = pi/2.
        [tOnOrb, polOnOrb, xyTraj] = integrateTraj(orbit, ...
            [ptOnOrbit(1) reachPhase], ...
            0:0.001:(4*pi/(abs(orbit.omega)))); %must take small steps if want high precision for pkPhase
                                               %took abs value on 22 mar. 2017 
                                                    
        %parse trajectory to figure out when hit pkPhase = pi/2
        if orbit.omega > 0
            pkPhase = pi/2;
            if reachPhase < pkPhase
                %find when polar theta coord. is close to pi/2
                ind2pk = ...
                    find(abs(polOnOrb(:,2) - pkPhase) < 0.01,1,'first');
                extraTime = tOnOrb(ind2pk);
            else
                %find when polar theta coord. is close to 2.5*pi
                ind2pk = ...
                    find(abs(polOnOrb(:,2) - (2*pi+pkPhase)) < 0.01,1,'first');
                extraTime = tOnOrb(ind2pk);
            end
        else
            ...
            if reachPhase < pkPhase
                ind2pk = ...
                    find(abs(polOnOrb(:,2) - (pkPhase - 2*pi)) < 0.01,1,'first');
                extraTime = tOnOrb(ind2pk);
            else
                ind2pk = ...
                    find(abs(polOnOrb(:,2) - (pkPhase)) < 0.01,1,'first');
        ...
        assert(numel(extraTime) == 1);
        assert(isscalar(extraTime));
