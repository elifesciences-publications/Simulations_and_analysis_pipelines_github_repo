...
preX = 0.05+[-3:0.25:3]; %0.05+[-3:0.25:3]; %gave error on X=1 for R=-3:1:3, %for X=2, messes  up on R = 3
preR = [-3:0.25:3]; %[-3:0.25:3];
...
%failed on r=3 (preR=-3:0.25:3,R=10^2.5), x=14 (0.5+[-3:0.25:3], X=10^0.3)
%for eccD=eccN=10. a = 10; didn't see anything obviously wrong by scanning
%the log, so restarted integration with ode23tb -- worked. Switch to this,
%worked for -3:1:3, -3:1:3. Will now try finely sampled.

%%% realized there was a problem with how circles were being considered in
%%% ellipseFminbnd, because had 1/R - delta < 10^-3, failed for R >= 10^3!
%%% Got crazey trajectories. Fixed the threshold to 10^-7. Try again. This
%%% should not have impacted the circle simulations. Try w. ode15s again --
%%% still got hung up. Try with ode23tb. Took 2.4 min to run r=1, x=1 with
%%% ode23b, but no crash. Traj looks circular. Will test 3:1:3 in both, if
%%% works, proceed to 3:0.25:3 on both axes.

%eccD=eccN=10 gave a very similar-looking heatmap to eccD=eccN=1. Try
%eccD=10, eccN=1. testing with R/X in [-3:1:3], [-3:1:3]. Integration
%finished without errors and relatively quickly: 90-120 sec per R,X combo.
%Also, this case looks a bit different from eccD=eccN=10. So run a
%finely-spaced R=[-3:0.25:3], X=[-3:0.25:3] simulation for eccD=10, eccN=1.

%how elliptical are the orbits?
eccD = 10; %1 = circle; try 1, 2 10.
eccN = 1;
...
for a=1:numel(aval)
    ...
    for r=1:numel(Rval)
        ...
        for x=1:numel(Xval)
            ...
            disp(['r=' num2str(r) ', x=' num2str(x) ...
                ' finished in ' num2str(tElapsed) ' min. ' char(10)]);
        end
    end
end
...
for a=1:numel(aval)
    ...
    title(['R_D/\delta_D=' num2str(eccD) ', ' 'R_N/\delta_N=' num2str(eccN)]);
    set(gca,'ydir','normal');
...
%     grid off;
    ...
    if TOEXP_HEATMAP == 1
        ...
        export_fig([getDate('yyyy-mm-dd') '_HeatMap_ellipse_' ...
