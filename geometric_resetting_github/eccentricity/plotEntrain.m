...
DAY_DELTA = SimVal.DAY_DELTA;
NIGHT_DELTA = SimVal.NIGHT_DELTA;
...
dO = makeEllipse(1, 0, DAY_DELTA, aval(pickA),OMSIGN*2*pi/24);
nO = makeEllipse(Rval(pickR), Xval(pickX), NIGHT_DELTA, aval(pickA), OMSIGN*2*pi/24);

[t, polTr, xyTr, dn] = ...
    entrain(dO, nO, tauval(pickTau), 24, NUMCYC, [dO.R theta0(pickTheta)]);
[tLL, polLL, release] = ...
    integrateTraj(dO, polarCoordOn(dO,xyTr(end,:)), ...
    0:0.1:squeeze(PEAKT(pickA,pickR,pickX,1,pickTau)));
...
if TOPLOT == 1
    ...
    %plot stable orbits
    plot(dCircle(:,1),dCircle(:,2), '-', 'color', dayCol, 'linewidth',2);
    hold on;
    plot(nCircle(:,1),nCircle(:,2), '-', 'color', nightCol, 'linewidth',2);
