...
dispif(0,['Using ' func2str(stepFun) ' in ' mfilename('fullpath') '.']);
...
dusk=[];
dawn=[];
dt = 0.05; %must set to <<0.1 because when driving with fractions of 26.7
...
dispif(0,['No. dusks: ' num2str(sum(dusk)) ', no. dawns: ' num2str(sum(dawn))]);
