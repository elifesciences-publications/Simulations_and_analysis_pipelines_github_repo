...
function [DAWNPHASE, DAWNPHASESHIFT, DUSKPHASE, DUSKPHASESHIFT] = ...
    drivePhaseOscilStepFn_WithBoot_PRC_Illustration(TEND, TLIGHT, TDARK, STEPUP, STEPDOWN, ...
    DPTIME, DPDUR, STARTPHASE)
%% TEST
TOTEST=1; %run in test mode?
if TOTEST==1
    ...
    DPTIME=4; %time when dark pulse is applied (hours)
    DPDUR=9; %duration of the dark pulse (hours)
    TEND=24*3; %how many hours to simulate
    STARTPHASE=0; %initial phase (at time=0)
end

%% export settings
TO_EXPORT_LDCYCLES_FIG = 1; %save the figure?
TOMAKELDCYCLES_FIG = 1; %do you want to make the figure illustrating phase resetting?
