...
pJan=errorbar(combPP(janGoodPlt),pkT(janGoodPlt),pkT_Err(janGoodPlt),...
    's', 'color', 'b', 'markersize',4,'markerfacecolor','b',...
        'markeredgecolor','none','linewidth',1);
...
pMar=errorbar(combPP(marGoodPlt),pkT(marGoodPlt),pkT_Err(marGoodPlt),...
    's', 'color', 'g', 'markersize',4,'markerfacecolor','g',...
        'markeredgecolor','none','linewidth',1);
...
set(fPkT_Compare,'units','inches','Position',[0 0 3.5 3.5],'color','w');

expif(TOEXP,{[getDate('yyyy-mm-dd') '_compEntr_eachExpOwnPer_' ...
    getDate('HH.MM.SS') '.pdf'],gcf});
...
dsAll = mat2dataset([gelTable; janTable; marTable]);
dsAll.Properties.VarNames = ds_names;
%export(dsAll,'FILE','in_vitro_entrainment.csv','Delimiter',',');
