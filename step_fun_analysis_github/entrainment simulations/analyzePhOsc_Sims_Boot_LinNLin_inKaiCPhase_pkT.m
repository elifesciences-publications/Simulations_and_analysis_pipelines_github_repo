%2017-06-26, EL: generate Fig. 4C, Fig. 4-figSup4A and source data for
%these panels.
...
SIMULATION_BOOT_NONLIN = load(['saved_data/2017-06-05_11.36.16_NonLin_FAST_4.mat']); 
SIMULATION_BOOT_LIN = load(['saved_data/2017-06-05_11.36.23_Lin_FAST_4.mat']);
...
%convert dawn phases to peak times
fvec = (@(x) (0.5-x)/(1/24));
...
if toexp_pts == 1
    ...
    ds = [];
    dsTable = [ones(size(vitro.pp(vitro.janGoodPlt)')) ...
        vitro.pp(vitro.janGoodPlt)' ...
        fvec(z*vitro.dawnPhase(vitro.janGoodPlt))' ...
        24*z*vitro.fitPhase_Err(vitro.janGoodPlt); ...
        1+ones(size(vitro.pp(vitro.marGoodPlt)')) ...
        vitro.pp(vitro.marGoodPlt)' ...
        fvec(z*vitro.dawnPhase(vitro.marGoodPlt))' ...
        24*z*vitro.fitPhase_Err(vitro.marGoodPlt) ];
    dsNames = {'trial','day_length_hrs','peak_time_in_PKaiC_units','peak_time_error'};
    ds = mat2dataset(dsTable);
    ds.Properties.VarNames = dsNames;
    export(ds,'FILE','Fig4C-in_vitro_entrainment_KaiCunits-June2017.csv','Delimiter',',');
end
...
if TOLOAD_OLD_SIMULATION_TWO == 1
    ...
    fracQuick = num1to3/numNet;
    fracQuick_notNan = num1to3/(numNet-numNan);
    disp([num2str(fracQuick) ' frac. entrained within 3 cycles in linear sims']);
    disp([num2str(fracQuick_notNan) ' frac. entrained w/in 3cyc out of nonNan in linear sims']);
